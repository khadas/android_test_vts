...
#   cd hallights/tests; mm hallights-unit-tests-glibc-run
# -----------------------------------------------------------------------------

ifeq ($(HOST_OS)-$(HOST_ARCH),$(filter $(HOST_OS)-$(HOST_ARCH),linux-x86 linux-x86_64))

hallights-unit-tests-glibc_src_files := \
    atexit_test.cpp \
    dlfcn_test.cpp \
    dl_test.cpp \
    pthread_dlfcn_test.cpp \

hallights-unit-tests-glibc_shared_libraries := \
    libdl_preempt_test_1 \
    libdl_preempt_test_2

hallights-unit-tests-glibc_shared_libraries += libdl_test_df_1_global

hallights-unit-tests-glibc_whole_static_libraries := \
    libHalLightsStandardTests \
    libVtsGtestMain \
    $(fortify_libs) \

hallights-unit-tests-glibc_static_libraries := \
    libbase \
    liblog \
    libcutils \

hallights-unit-tests-glibc_ldlibs := \
    -lrt -ldl -lutil \

hallights-unit-tests-glibc_c_includes := \
    bionic/libc \

hallights-unit-tests-glibc_cflags := $(test_cflags)
hallights-unit-tests-glibc_cppflags := $(test_cppflags)
hallights-unit-tests-glibc_ldflags := -Wl,--export-dynamic

hallights-unit-tests-glibc_allow_asan := true

module := hallights-unit-tests-glibc
module_tag := optional
build_type := host
build_target := NATIVE_TEST
include $(LOCAL_PATH)/../Android.build.mk
...
LOCAL_CLANG := false
LOCAL_MODULE := hallights-compile-time-tests-g++
LOCAL_CPPFLAGS := -Wall
...
LOCAL_CLANG := true
LOCAL_MODULE := hallights-compile-time-tests-clang++
LOCAL_CPPFLAGS := -Wall
# Disable color diagnostics so the warnings output matches the source
...
# HALLIGHTS_TEST_FLAGS is either empty or it comes from the user.
.PHONY: hallights-unit-tests-glibc-run
hallights-unit-tests-glibc-run: hallights-unit-tests-glibc
	mkdir -p $(TARGET_OUT_DATA)/local/tmp
	ANDROID_DATA=$(TARGET_OUT_DATA) \
	ANDROID_ROOT=$(TARGET_OUT) \
		$(HOST_OUT_EXECUTABLES)/hallights-unit-tests-glibc64 $(HALLIGHTS_TEST_FLAGS)

# -----------------------------------------------------------------------------
# Run the unit tests built against x86 hallights on an x86 host.
# -----------------------------------------------------------------------------

include $(LOCAL_PATH)/../run-on-host.mk
...
# HALLIGHTS_TEST_FLAGS is either empty or it comes from the user.
.PHONY: hallights-unit-tests-run-on-host32
hallights-unit-tests-run-on-host32: hallights-unit-tests hallights-prepare-run-on-host
	ANDROID_DATA=$(TARGET_OUT_DATA) \
	ANDROID_DNS_MODE=local \
	ANDROID_ROOT=$(TARGET_OUT) \
		timeout $(TEST_TIMEOUT) \
		$(TARGET_OUT_DATA)/nativetest/hallights-unit-tests/hallights-unit-tests32 $(HALLIGHTS_TEST_FLAGS)

ifeq ($(TARGET_IS_64_BIT),true)
# add target to run lp64 tests
.PHONY: hallights-unit-tests-run-on-host64
hallights-unit-tests-run-on-host64: hallights-unit-tests hallights-prepare-run-on-host
	ANDROID_DATA=$(TARGET_OUT_DATA) \
	ANDROID_DNS_MODE=local \
	ANDROID_ROOT=$(TARGET_OUT) \
		timeout $(TEST_TIMEOUT) \
		$(TARGET_OUT_DATA)/nativetest64/hallights-unit-tests/hallights-unit-tests64 $(HALLIGHTS_TEST_FLAGS)
endif

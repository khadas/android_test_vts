...
class VtsStructuralTestHalPowerTest : public ::testing::Test {
 ...
 protected:
  VtsStructuralTestHalPowerTest() {
    int rc = hw_get_module(POWER_HARDWARE_MODULE_ID, (hw_module_t const**)&module_);
    if (rc || !module_) {
...
TEST_F(VtsStructuralTestHalPowerTest,
  ...
  EXPECT_EQ(num_modes_, rc2) << "get_number_of_platform_modes returned unequal values";
}
...
TEST_F(VtsStructuralTestHalPowerTest, check_if_the_values_decrease) {
  ...
  power_state_platform_sleep_state_t *list1 = (power_state_platform_sleep_state_t *)
      calloc(num_modes_, sizeof(power_state_platform_sleep_state_t));
  ASSERT_TRUE(list1) << "power_state_platform_sleep_state_t* alloc failed";

  power_state_platform_sleep_state_t *list2 = (power_state_platform_sleep_state_t *)
      calloc(num_modes_, sizeof(power_state_platform_sleep_state_t));
  ...
  for (int i = 0; i < num_modes_; i++) {
    list1[i].voters = (power_state_voter_t *) calloc(voter_list[i],
        sizeof(power_state_voter_t));
    ASSERT_TRUE(list1[i].voters) << "voter_t allocation failed";

    list2[i].voters = (power_state_voter_t *) calloc(voter_list[i],
        sizeof(power_state_voter_t));
